...
byte* hexStringToBytes(String const &hexString, int &resultLength)
{
    ...
    // Allocate and zero buffer for bytes
    resultLength = hexString.length()*sizeof(byte)/2;
    buffer = (byte*)malloc(resultLength);

    if (buffer)
    {
      if (hexStringToBytes(hexString, buffer, resultLength, resultLength))
      {
        return buffer;
      }
      else
...
boolean hexStringToBytes(String const &hexString, byte *buffer, int &resultLength, int maxLength)
{
    ...
    if (hexString.charAt(0) == 'O' && hexString.charAt(1) == 'x')
    {
        i = 2;
    }
